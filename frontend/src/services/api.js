...
export function mapTask(dto) {
  ...
  return {
    ...
    startDateTime: dto.startDateTime
      ? new Date(dto.startDateTime).toLocaleString()
      : (dto.createdAt ? new Date(dto.createdAt).toLocaleString() : 'N/A'),
...
export async function fetchFormData(urlPath, options = {}) {
  let sessionUser = null;
  try {
    const rawUser = localStorage.getItem('cloudkaptan_user') || localStorage.getItem('finsop_session');
    if (rawUser) {
...
export async function createCompleteSop(sopData) {
  const res = await fetchJson('/sops/create-complete', {
    method: 'POST',
    body: JSON.stringify(sopData),
  }).catch(() => null);
  return res?.data || res;
...
export async function reAssignTask(taskId, actorId, comment, makerIds = [], checkerIds = []) {
  let id = taskId;
  let actor = actorId;
  let comm = comment;
  ...
  const res = await fetchJson(`/tasks/${id}/reassign`, {
    method: 'PUT',
    body: JSON.stringify({ actorId: actor, makerIds, checkerIds, reason: comm }),
  }).catch(() => null);

  return res;
}

export async function uploadDocumentsToTask(taskId = '', actorId = '', formData) {
  const params = new URLSearchParams();
  if (actorId) params.append('actorId', actorId);
  const query = params.toString() ? `?${params.toString()}` : '';

  const response = await fetchFormData(`/tasks/${taskId}/documents${query}`, {
    method: 'POST',
    body: formData,
  }).catch(() => null);
  return response;
...
export async function viewUploadedDocumentsInTask(taskId) {
  const response = await fetchJson(`/tasks/${taskId}/documents`).catch(() => null);
  return response;
}

export async function downloadDocumentInTask(taskId, documentId, actorId) {
  const params = new URLSearchParams();
  if (actorId) params.append('actorId', actorId);
  const query = params.toString() ? `?${params.toString()}` : '';

  const response = await fetchJson(`/tasks/${taskId}/documents/${documentId}${query}`).catch(() => null);
  return response;
}

...
// Embed MOCK_TASKS
const startTasksIndex = apiContent.indexOf('export const MOCK_TASKS = [');
const endTasksIndex = apiContent.indexOf('];\n\nexport const MOCK_ORGANIZATION_USERS');

if (startTasksIndex !== -1 && endTasksIndex !== -1) {
  const newTasksStr = `export const MOCK_TASKS = ${JSON.stringify(generatedTasks, null, 2)}`;
  apiContent = apiContent.slice(0, startTasksIndex) + newTasksStr + apiContent.slice(endTasksIndex);
}

// Embed MOCK_AUDIT_LOGS
const startAuditIndex = apiContent.indexOf('export const MOCK_AUDIT_LOGS = [');
const endAuditIndex = apiContent.indexOf('];\n\nexport async function fetchJson');

if (startAuditIndex !== -1 && endAuditIndex !== -1) {
  const newAuditStr = `export const MOCK_AUDIT_LOGS = ${JSON.stringify(generatedAuditLogs, null, 2)}`;
  apiContent = apiContent.slice(0, startAuditIndex) + newAuditStr + apiContent.slice(endAuditIndex);
}

fs.writeFileSync(apiFilePath, apiContent, 'utf8');
console.log(`Successfully generated ${generatedTasks.length} tasks and ${generatedAuditLogs.length} audit logs in api.js!`);

...
const tasksHeader = 'export const MOCK_TASKS = [';
const tasksEndMarker = '];\n\nexport const MOCK_ORGANIZATION_USERS';

const tStart = apiContent.indexOf(tasksHeader);
const tEnd = apiContent.indexOf(tasksEndMarker, tStart);

if (tStart !== -1 && tEnd !== -1) {
  const replacement = `export const MOCK_TASKS = ${JSON.stringify(generatedTasks, null, 2)}`;
  apiContent = apiContent.substring(0, tStart) + replacement + apiContent.substring(tEnd + 2);
}

// Replace MOCK_AUDIT_LOGS array
const auditHeader = 'export const MOCK_AUDIT_LOGS = [';
const auditEndMarker = '];\n\nexport async function fetchJson';

const aStart = apiContent.indexOf(auditHeader);
const aEnd = apiContent.indexOf(auditEndMarker, aStart);

if (aStart !== -1 && aEnd !== -1) {
  const replacement = `export const MOCK_AUDIT_LOGS = ${JSON.stringify(generatedAuditLogs, null, 2)}`;
  apiContent = apiContent.substring(0, aStart) + replacement + apiContent.substring(aEnd + 2);
}

fs.writeFileSync(apiFilePath, apiContent, 'utf8');
console.log(`Injected ${generatedTasks.length} tasks and ${generatedAuditLogs.length} audit logs into api.js!`);
